...
package AxmlEditor.rt;

import AxmlEditor.rt.ResConst;
import AxmlEditor.rt.StringItems;
...
public class Parser {
    ...
    public int next() throws IOException {
        ...
        for (int p = in.position(); p < fileSize; p = in.position()) {
            ...
            int size = in.getInt();
            switch (type) {
                case ResConst.RES_XML_START_ELEMENT_TYPE: {
                        {
                    ...
                    break;
                case ResConst.RES_XML_END_ELEMENT_TYPE: {
                        in.position(p + size);
                        event = END_TAG;
                    }
                    break;
                case ResConst.RES_XML_START_NAMESPACE_TYPE:
                    lineNumber = in.getInt();
                    ...
                    event = START_NS;
                    break;
                case ResConst.RES_XML_END_NAMESPACE_TYPE:
                    in.position(p + size);
                    event = END_NS;
                    break;
                case ResConst.RES_STRING_POOL_TYPE:
                    strings = StringItems.read(in);
                    in.position(p + size);
                    continue;
                case ResConst.RES_XML_RESOURCE_MAP_TYPE:
                    int count = size / 4 - 2;
                    ...
                    in.position(p + size);
                    continue;
                case ResConst.RES_XML_CDATA_TYPE:
                    lineNumber = in.getInt();
                    in.getInt();/* 0xFFFFFFFF */
                    textIdx = in.getInt();
